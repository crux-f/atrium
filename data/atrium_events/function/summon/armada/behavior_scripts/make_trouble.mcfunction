...
execute at @s[scores={charge=0..6}] if predicate atrium:percentage_chances/0.25_p run function atrium_events:summon/armada/behavior_scripts/rewire_golem
#
# If their score is 7-12, they have a 15% chance of permenantly hijacking a local iron golem.
execute at @s[scores={charge=7..12}] if predicate atrium:percentage_chances/0.15_p run function atrium_events:summon/armada/behavior_scripts/hijack_golem
#
# If their score is 13+, they have a 10% chance of promoting a Sleeper Agent to a Secret Police.
execute if entity @s[scores={charge=13..}] if predicate atrium:percentage_chances/0.10_p run function atrium_events:summon/armada/behavior_scripts/promote_sleeper_agent
#
