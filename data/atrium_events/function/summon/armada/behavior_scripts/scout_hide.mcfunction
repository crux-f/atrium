...
execute as @e[tag=atrium_armada_scout,scores={hiding=30}] run effect give @s invisibility infinite 0 false
execute as @e[tag=atrium_armada_scout,scores={hiding=30}] at @s run playsound minecraft:entity.wandering_trader.disappeared hostile @a[distance=..10] ~ ~ ~ 1 1 0.1
#
# come out of hiding if in battle
# Leader gets a spyglass
execute as @e[tag=atrium_armada_scout,tag=atrium_scouting_party_leader,scores={hiding=30..}] at @s if entity @a[distance=..10,gamemode=survival] run data merge entity @s {Silent:0b,PersistenceRequired:1b,equipment:{mainhand:{id:"minecraft:crossbow",count:1,tag:{display:{Name:{text:"Scout\s Crossbow","italic":false}},scouts_crossbow:1b,Enchantments:[{id:"minecraft:unbreaking",lvl:1s},offhand:{id:"minecraft:quick_charge",lvl:1s}},AttributeModifiers:[{AttributeName:"generic.movement_speed",Name:"generic.movement_speed",Amount:0.08,Operation:1,UUID:[I;-1816239232,-243186249,-2029629323,-1291332108],Slot:"mainhand"}]}},{id:"minecraft:spyglass",count:1,tag:{display:{Name:{text:"Spy-Sailor\s Spyglass","italic":false}},atrium_spy_sailors_spyglass:1b,AttributeModifiers:[{AttributeName:"generic.follow_range",Name:"generic.follow_range",Amount:0.25,Operation:1,UUID:[I;95612904,-2063644374,-1922784920,400963316],Slot:"offhand"}]}}],HandDropChances:[0.650F,0.100F]}
...
execute as @e[tag=atrium_armada_scout,scores={hiding=30..}] at @s if entity @a[distance=..10,gamemode=survival] run playsound minecraft:entity.wandering_trader.reappeared hostile @a[distance=..10] ~ ~ ~ 1 1 0.1
