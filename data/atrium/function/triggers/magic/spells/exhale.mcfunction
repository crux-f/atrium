...
execute anchored eyes positioned ^ ^ ^6 as @e[distance=..3] run tag @s add atrium_exhale_target
#
execute as @e[tag=atrium_exhale_target,distance=..25] store result score @s atrium_x_coordinate run data get entity @s Pos[0] 50
execute as @e[tag=atrium_exhale_target,distance=..25] store result score @s atrium_y_coordinate run data get entity @s Pos[1] 50
execute as @e[tag=atrium_exhale_target,distance=..25] store result score @s atrium_z_coordinate run data get entity @s Pos[2] 50
# Subtract the target's coordinates from caster's coordinates, then set the target's coordinate scores to the result
scoreboard players operation @e[tag=atrium_exhale_target,distance=..25] atrium_x_coordinate -= @s atrium_x_coordinate
...
execute at @s[nbt={equipment:{offhand:{components:{"minecraft:custom_data":{atrium_staff_lv:2b}}}}}] as @e[tag=atrium_exhale_target,distance=..25] store result entity @s Motion[0] double 0.02 run scoreboard players get @s atrium_x_coordinate
execute at @s[nbt={equipment:{offhand:{components:{"minecraft:custom_data":{atrium_staff_lv:2b}}}}}] as @e[tag=atrium_exhale_target,distance=..25] store result entity @s Motion[1] double 0.02 run scoreboard players get @s atrium_y_coordinate
execute at @s[nbt={equipment:{offhand:{components:{"minecraft:custom_data":{atrium_staff_lv:2b}}}}}] as @e[tag=atrium_exhale_target,distance=..25] store result entity @s Motion[2] double 0.02 run scoreboard players get @s atrium_z_coordinate
...
# Remove tag
tag @e[tag=atrium_exhale_target] remove atrium_exhale_target 
#
#
# Spell cleanup
