...
advancement revoke @s only atrium:triggers/chests/unlock/village_desert_house_chest
# Tag player
tag @s add village_desert_house_key_remove
# Remove key in 1 tick (allows for loot table to have a chance to run before the key is removed)
schedule function atrium:triggers/chests/take_key/village_desert_house_key 1t
