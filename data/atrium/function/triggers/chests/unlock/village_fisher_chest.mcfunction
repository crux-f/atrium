...
advancement revoke @s only atrium:triggers/chests/unlock/village_fisher_chest
# Tag player
tag @s add village_fisher_key_remove
# Remove key in 1 tick (allows for loot table to have a chance to run before the key is removed)
schedule function atrium:triggers/chests/take_key/village_fisher_key 1t
