...
execute at @s positioned ^ ^ ^3 at @e[limit=1,type=minecraft:wandering_trader,distance=..3] unless entity @e[tag=trader_defense_golem,distance=..10] run summon iron_golem ~ ~ ~ {DeathLootTable:"minecraft:empty",PersistenceRequired:1b,Health:200f,PlayerCreated:0b,AngerTime:999999999,Tags:["trader_defense_golem"],ActiveEffects:[{Id:5,Amplifier:0b,Duration:2400,ShowParticles:1b}],Attributes:[{Name:"generic.max_health",Base:200},{Name:"generic.follow_range",Base:80},{Name:"generic.movement_speed",Base:0.35}]}
