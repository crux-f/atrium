...
advancement revoke @s only atrium:triggers/items/quest_maps/mansion_quest_complete
# Tag player
tag @s add atrium_getting_mansion_reward
# Remove key in 1 tick (allows for loot table to have a chance to run before the key is removed)
schedule function atrium:triggers/items/quest_maps/take_map/mansion_quest 1t
