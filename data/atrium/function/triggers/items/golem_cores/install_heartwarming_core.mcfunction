...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,type=minecraft:iron_golem,sort=nearest,distance=..5] if entity @s[tag=atrium_golem_core_installed] run give @p[gamemode=!spectator] minecraft:iron_ingot[minecraft:custom_model_data={strings:["atrium_golem_core_heartwarming"]},minecraft:lore=['{"text":"Heartwarming","color":"gold","italic":false}','{"text":" "}','{"text":"When Used on Damaged Iron Golem:","color":"dark_purple","italic":false}','{"text":"Cold Resistance (∞)","color":"blue","italic":false}'],minecraft:item_name='{"text":"Golem Core"}',minecraft:custom_data={atrium_core_type:"heartwarming",atrium_golem_core:1b}] 1
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run data modify entity @s ArmorItems[0] set value {id:"minecraft:leather_boots",count:1,components:{"minecraft:unbreakable":{}}}
