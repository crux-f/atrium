...
advancement revoke @s only atrium:triggers/items/golem_cores/install_windy_core
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,type=minecraft:iron_golem,sort=nearest,distance=..5] if entity @s[tag=atrium_golem_core_installed] run give @p[gamemode=!spectator] minecraft:iron_ingot[minecraft:custom_model_data={strings:["atrium_golem_core_windy"]},minecraft:lore=['{"text":"Windy","color":"blue","italic":false}','{"text":" "}','{"text":"When Used on damaged Iron Golem:","color":"dark_purple","italic":false}','{"text":"+1 Step Height","color":"blue","italic":false}'],minecraft:item_name='{"text":"Golem Core"}',minecraft:custom_data={atrium_core_type:"windy",atrium_golem_core:1b}] 1
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run data merge entity @s {ArmorItems:[{id:"minecraft:iron_ingot",count:1,components:{"minecraft:attribute_modifiers":[{id:"atrium:windy_core_step_height",type:"step_height",amount:1,operation:"add_value",slot:"feet"}],"minecraft:item_name":'"Windy Golem Shoes"'}},{},{},{}],ArmorDropChances:[-327.670F,0.085F,0.085F,0.085F]}
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run effect give @s minecraft:wind_charged infinite
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run tag @s add atrium_windy_core_installed
