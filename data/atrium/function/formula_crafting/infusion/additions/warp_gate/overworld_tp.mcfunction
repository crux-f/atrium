...
execute at @e[tag=fx_travel] positioned ~-6 ~0.5 ~ run particle minecraft:happy_villager ~ ~ ~ 0.25 0.5 0.25 0.1 2
...
#
# Teleport!
execute in minecraft:overworld at @e[tag=fx_travel] positioned ~-6 ~ ~ as @e[distance=..1,type=!item_frame,type=!marker,tag=!atrium_currently_warping] at @s run function atrium:formula_crafting/infusion/additions/warp_gate/summon_warp_gate_hook
#
...
execute at @e[tag=fx_travel,scores={warp_gate=..1200}] positioned ~-5 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{components:{"minecraft:lodestone_tracker":{}}}}] run schedule function atrium:formula_crafting/infusion/additions/warp_gate/overworld_tp 1t
#
