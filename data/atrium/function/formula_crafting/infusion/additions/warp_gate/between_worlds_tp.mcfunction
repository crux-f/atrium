...
execute at @e[tag=fx_travel] positioned ~-6 ~0.5 ~ run particle minecraft:effect ~ ~ ~ 0.15 0.5 0.15 0.1 10
...
execute at @e[tag=fx_travel,scores={warp_gate=..1200}] positioned ~-5 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{components:{"minecraft:lodestone_tracker":{"tracked":true}}}}] run schedule function atrium:formula_crafting/infusion/additions/warp_gate/between_worlds_tp 1t
#
