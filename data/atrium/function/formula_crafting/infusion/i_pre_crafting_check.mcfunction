...
execute positioned ~-6 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5] run scoreboard players add @s valid_ingredients 1
...
execute if entity @s[scores={valid_ingredients=..24}] if data entity @s data.warp_gate_installed run function atrium:formula_crafting/infusion/err_gate_broken
#
# Warp Gate
#
# If you have the Gate installed and the codex has 25 or higher, the check passes.
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=25..}] if data entity @s data.warp_gate_installed run function atrium:formula_crafting/infusion/infusion_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=25..}] if data entity @s data.warp_gate_installed run function atrium:items/use/placeable_structures/infusion_circle/pack_infusion_circle
#
# Regular
#
...
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=12..}] unless data entity @s data.warp_gate_installed run function atrium:formula_crafting/infusion/infusion_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=12..}] unless data entity @s data.warp_gate_installed run function atrium:items/use/placeable_structures/infusion_circle/pack_infusion_circle
#
# Reset Score
