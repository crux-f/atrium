...
execute if entity @s[scores={valid_ingredients=5..}] as @n[type=item,tag=atrium_formula_product,distance=..7] run data merge entity @s {Item:{components:{"minecraft:enchantments":{"atrium:sentinel": 1}}}}
...
execute if entity @s[scores={valid_ingredients=5..}] as @r[distance=..10,level=15..,gamemode=!spectator] run xp add @s -2 levels
