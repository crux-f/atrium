...
#
# Check NORTH EAST for Prismarine Crystals
...
#
# Check SOUTH WEST for Lightning Rod
execute positioned ~-2 ~ ~2 at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:lightning_rod",count:1}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~-2 ~ ~2 if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:lightning_rod",count:1}}] run scoreboard players add @s valid_ingredients 1
#
# Check SOUTH EAST for Lightning Rod
execute positioned ~2 ~ ~2 at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:lightning_rod",count:1}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~ ~2 if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:lightning_rod",count:1}}] run scoreboard players add @s valid_ingredients 1
#
...
execute if entity @s[scores={valid_ingredients=5..}] at @s run item replace entity @n[type=item,tag=atrium_formula_product,distance=..7] container.0 with minecraft:enchanted_book[minecraft:stored_enchantments={"minecraft:channeling":1}]
...
# Take 4 levels of XP from a random nearby player (within 10 blocks) who has at least 25 levels of XP
execute if entity @s[scores={valid_ingredients=5..}] as @r[distance=..10,level=25..,gamemode=!spectator] run xp add @s -4 levels
