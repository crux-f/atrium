...
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=13..}] if data entity @s data.occult_altar_installed run function atrium:formula_crafting/enchantment/enchantment_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=13..}] if data entity @s data.occult_altar_installed run function atrium:items/use/placeable_structures/runic_codex/pack_runic_codex
#
# Regular
#
# If the codex has 9 or higher, the check passes.
# UNLESS the Occult Altar upgrade is installed. If it is, we need to check more.
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=9..}] unless data entity @s data.occult_altar_installed run function atrium:formula_crafting/enchantment/enchantment_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=9..}] unless data entity @s data.occult_altar_installed run function atrium:items/use/placeable_structures/runic_codex/pack_runic_codex
#
#
# Reset Score
