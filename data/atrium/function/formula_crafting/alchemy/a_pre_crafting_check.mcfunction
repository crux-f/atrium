...
execute if entity @s[scores={valid_ingredients=..8}] if entity @s[tag=atrium_cooking_station_installed] run function atrium:formula_crafting/alchemy/err_station_broken
#
# Cooking Station
#
# If you have the Station installed and the cauldron has 9 or higher, the check passes.
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=9..}] if entity @s[tag=atrium_cooking_station_installed] run function atrium:formula_crafting/alchemy/alchemy_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=9..}] if entity @s[tag=atrium_cooking_station_installed] run function atrium:items/use/placeable_structures/alchemy_cauldron/pack_alchemy_cauldron
#
# Regular
#
# If the cauldron has 5 or higher, the check passes.
#
execute unless entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}}] if entity @s[scores={valid_ingredients=5..}] unless entity @s[tag=atrium_cooking_station_installed] run function atrium:formula_crafting/alchemy/alchemy_base_item
# If the player is holding a Rod of Relocation, run the pack it up script instead.
execute if entity @p[gamemode=!spectator,distance=..6,nbt={SelectedItem:{components:{"minecraft:custom_data":{atrium_rod_of_relocation:1b}}}},level=5..] if entity @s[scores={valid_ingredients=5..}] unless entity @s[tag=atrium_cooking_station_installed] run function atrium:items/use/placeable_structures/alchemy_cauldron/pack_alchemy_cauldron
#
# Reset Score
