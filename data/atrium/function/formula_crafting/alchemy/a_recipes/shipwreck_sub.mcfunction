...
execute if entity @s[scores={valid_ingredients=5..}] at @s run item replace entity @n[type=item,tag=atrium_formula_product,distance=..7] container.0 with minecraft:bread[minecraft:custom_model_data={strings:["atrium_shipwreck_sub"]},minecraft:item_name={text:'Shipwreck Sub'},minecraft:custom_data={atrium_shipwreck_sub:1b},minecraft:food={nutrition:8,saturation:8},minecraft:consumable={consume_seconds:3,animation:"eat",has_consume_particles:true}]
...
# Give items back
execute if entity @s[scores={valid_ingredients=5..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.0,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:glass_bottle"}}
