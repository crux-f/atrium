...
execute if entity @s[scores={valid_ingredients=5..}] at @s run item replace entity @n[type=item,tag=atrium_formula_product,distance=..7] container.0 with minecraft:baked_potato[minecraft:custom_model_data={strings:["atrium_yellow_cheese"]},minecraft:item_name={text:'Yellow Cheese'},minecraft:custom_data={atrium_yellow_cheese:1b}]
...
# Give items back
execute if entity @s[scores={valid_ingredients=5..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.0,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:yellow_wool"}}
