...
#
execute if entity @s[scores={valid_ingredients=3..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:glowstone",components:{"minecraft:enchantment_glint_override":true,"minecraft:lore":[{"text":"Galvanizing","color":"yellow","italic":false},{"text":" "},{"text":"When energized by magma,"},{"text":"glowstone has the ability"},{"text":"to stabilize volatile"},{"text":"potion combinations."}],"minecraft:item_name":{text:'Alchemy Catalyst'},"minecraft:custom_data":{atrium_catalyst_galvanizing:1b}}}}
