...
execute positioned ~-2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg"}}] run scoreboard players add @s valid_ingredients 1
# Check EAST for Egg
execute positioned ~2 ~ ~ at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg"}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg"}}] run scoreboard players add @s valid_ingredients 1
...
# Check Crafting Station for Milk Bucket
...
execute if entity @s[scores={valid_ingredients=5..}] at @s run item replace entity @n[type=item,tag=atrium_formula_product,distance=..7] container.0 with minecraft:potion[minecraft:custom_model_data={strings:["atrium_small_bottle"]},minecraft:item_name={text:'Mayo Bottle'},minecraft:custom_data={atrium_mayo_bottle:1b},minecraft:potion_contents={custom_color:16383912,custom_effects:[{amplifier:0b,duration:2400,id:"minecraft:nausea",show_icon:1b,show_particles:1b}]},minecraft:tooltip_display={hidden_components:["minecraft:potion_contents"]}]
...
# Give items back
execute if entity @s[scores={valid_ingredients=5..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.0,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:bucket"}}
