...
#
execute if entity @s[scores={valid_ingredients=3..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:potion",components:{"minecraft:custom_name":{text:'Potion of Slow Falling',italic:false},"minecraft:custom_model_data":{strings:["atrium_advanced_potion"]},"minecraft:custom_data":{atrium_tier_3_potion:1b,atrium_potion:"longer_slow_falling"},"minecraft:potion_contents":{custom_effects:[{id:"minecraft:slow_falling",amplifier:0,duration:9600}]}}}}
