...
# execute as @p[distance=..8,gamemode=!spectator] unless entity @s[nbt={SelectedItem:{id:"minecraft:golden_apple"}}] unless entity @s[nbt={SelectedItem:{id:"minecraft:enchanted_golden_apple"}}] run function atrium:shrines/blessings/oswald_prayer
execute as @p[distance=..8,gamemode=!spectator] run function atrium:shrines/blessings/oswald_prayer
