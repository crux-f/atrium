...
execute unless entity @s[type=player] as @p[distance=..8,gamemode=!spectator] at @s run function atrium:shrines/blessings/nameless_prayer
#
...
execute if entity @s[type=player] at @s at @n[type=item_display,tag=atrium_nameless_shrine_display] run function atrium:shrines/blessings/nameless_prayer
#
# If using system B, remove the trigger advancement so this can trigger again in the future.
execute if entity @s[type=player] run advancement revoke @s only atrium:triggers/shrines/activate_nameless_shrine
#
