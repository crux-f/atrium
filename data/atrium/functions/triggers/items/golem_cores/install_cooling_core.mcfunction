...
advancement revoke @s only atrium:triggers/items/golem_cores/install_cooling_core
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,type=minecraft:iron_golem,sort=nearest,distance=..5] if entity @s[tag=atrium_golem_core_installed] run give @p[gamemode=!spectator] minecraft:iron_ingot{display:{Name:'{"text":"Golem Core","italic":false}',Lore:['{"text":"Cooling","color":"aqua","italic":false}','{"text":" "}','{"text":"When Used on Iron Golem:","color":"dark_purple","italic":false}','{"text":"Fire Resistance (∞)","color":"blue","italic":false}']},CustomModelData:333155,atrium_golem_core:1b,atrium_core_type:cooling} 1
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run effect give @s minecraft:fire_resistance infinite
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run tag @s add atrium_cooling_core_installed
