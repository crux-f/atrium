...
advancement revoke @s only atrium:triggers/items/golem_cores/install_empathic_core
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,type=minecraft:iron_golem,sort=nearest,distance=..5] if entity @s[tag=atrium_golem_core_installed] run give @p[gamemode=!spectator] minecraft:iron_ingot{display:{Name:'{"text":"Golem Core","italic":false}',Lore:['{"text":"Empathic","color":"light_purple","italic":false}','{"text":" "}','{"text":"When Used on Damaged Iron Golem:","color":"dark_purple","italic":false}','{"text":"+4 Health to Player on Repair","color":"blue","italic":false}']},CustomModelData:333156,atrium_golem_core:1b,atrium_core_type:empathic} 1
...
#
# The Empathic core functions based on an advancement, so the tag is all that's needed for the ability to work.
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run tag @s add atrium_empathic_core_installed
