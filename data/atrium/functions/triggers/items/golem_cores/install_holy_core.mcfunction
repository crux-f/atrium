...
advancement revoke @s only atrium:triggers/items/golem_cores/install_holy_core
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run data merge entity @s {HandItems:[{id:"minecraft:iron_ingot",Count:1b,tag:{HandItems:[{id:"minecraft:iron_ingot",Count:1b,tag:{display:{Name:'{"text":"Holy Core Attack","color":"yellow","italic":false}'},Enchantments:[{id:"minecraft:smite",lvl:2s}]}},{}],HandDropChances:[0.000F,0.085F]}}]}
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run tag @s add atrium_holy_core_installed
