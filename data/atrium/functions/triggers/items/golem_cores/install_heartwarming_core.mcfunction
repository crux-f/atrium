...
advancement revoke @s only atrium:triggers/items/golem_cores/install_heartwarming_core
#
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,type=minecraft:iron_golem,sort=nearest,distance=..5] if entity @s[tag=atrium_golem_core_installed] run give @p[gamemode=!spectator] minecraft:iron_ingot{display:{Name:'{"text":"Golem Core","italic":false}',Lore:['{"text":"Overclocked","color":"red","italic":false}','{"text":" "}','{"text":"When Used on Damaged Iron Golem:","color":"dark_purple","italic":false}','{"text":"Speed (∞)","color":"blue","italic":false}']},CustomModelData:333160,atrium_golem_core:1b,atrium_core_type:overclocked} 1
...
#
# Grand Iron Golem the ability associated with the core type
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run data modify entity @s ArmorItems[0] set value {id:"minecraft:leather_boots",Count:1b,tag:{Unbreakable:1b}} 
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run data modify entity @s ArmorDropChances[0] set value 0f
...
execute at @s positioned ^ ^ ^1.5 as @e[limit=1,tag=atrium_installing_core,sort=nearest,distance=..5] run tag @s add atrium_heartwarming_core_installed
