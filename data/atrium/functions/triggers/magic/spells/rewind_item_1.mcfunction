...
execute as @a[tag=atrium_rewind_item_caster] at @s if data entity @s {SelectedItem:{tag:{Enchantments:[{id:"minecraft:vanishing_curse",lvl:5s}]}}} run function atrium:triggers/magic/spell_failure_effects
execute as @a[tag=atrium_rewind_item_caster] at @s if data entity @s {SelectedItem:{tag:{Enchantments:[{id:"minecraft:vanishing_curse",lvl:5s}]}}} run tellraw @s {"text":"(Even miracles only go so far.)","italic":true,"color":"gray"}
