...
execute positioned ~2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{catalyst_inscribe:1b}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{catalyst_inscribe:1b}}}] run scoreboard players add @s infusion 1
#
# Check CENTER WEST for Enchantment Catalyst (Perpetuating). If true, construct the WARP GATE.
execute positioned ~-2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{catalyst_perpetuate:1b}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
#
# Tag the nearest infusion marker with warp_gate
execute if entity @s[scores={infusion=8..}] at @s run tag @e[tag=infusion,distance=..10] add warp_gate
#
# Tag the nearest warp_gate with the temporary build FX tag
execute if entity @s[scores={infusion=8..}] at @s run tag @e[tag=warp_gate,distance=..10] add fx_build
#
# Run the build script
execute if entity @s[scores={infusion=8..}] at @s run function atrium:travel/warp_gate/warp_gate_build
#
# Reset
execute if entity @s[scores={infusion=8..}] at @s run function atrium:magic/infusion/infusion_reset
