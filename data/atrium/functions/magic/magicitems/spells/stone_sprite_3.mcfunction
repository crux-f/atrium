# AS all players that meet the spell requirements run particles
execute at @s anchored eyes run particle minecraft:happy_villager ^-0.4 ^-0.05 ^0.5 0 0 0 0 4
# Summon a stone sprite at your feet!
execute at @s run summon area_effect_cloud ~ ~ ~ {Particle:"mycelium",Radius:0.5f,RadiusPerTick:0f,RadiusOnUse:0f,Duration:960,Potion:"minecraft:empty",Effects:[{Id:3,Amplifier:1b,Duration:9600,ShowParticles:1b}]}
# Sound effect
execute at @s run playsound minecraft:entity.bat.takeoff player @a[distance=..30] ~ ~ ~ 0.3 0.9 0.1
clear @s minecraft:glowstone_dust 1
# Spell cleanup
execute as @s run function atrium:magic/magicitems/spell_cast_complete
