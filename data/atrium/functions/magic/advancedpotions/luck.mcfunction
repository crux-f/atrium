...
#
# Check EAST for mixer item.
# THE THIRD ITEM ALWAYS HAS TO BE ..2 FOR DUMB REASONS DO NOT CHANGE THIS OR I WILL STEAL YOUR SHOES
execute positioned ~2 ~2 ~ at @e[type=minecraft:item_frame,distance=..2,nbt={Item:{tag:{comet_berries:1b}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..2,nbt={Item:{tag:{comet_berries:1b}}}] run scoreboard players add @s cauldron 1
#
# Check NORTH for special ingredient
...
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~2 ~ {NoGravity:1b,Glowing:1b,Motion:[0.0,0.05,0.0],Item:{id:"minecraft:potion",Count:1b,tag:{Potion:"minecraft:luck"}}}
execute if entity @s[scores={cauldron=3..}] at @s run execute positioned ~ ~ ~-4 run advancement grant @p[distance=..10] only atrium:secret_ingredient
#
