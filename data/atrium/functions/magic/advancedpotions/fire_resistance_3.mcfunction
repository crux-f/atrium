...
#
# Check ???
execute positioned ~2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{primordial_stone:1b}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{primordial_stone:1b}}}] run scoreboard players add @s cauldron 1
#
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~2 ~ {NoGravity:1b,Glowing:1b,Motion:[0.0,0.05,0.0],Item:{id:"minecraft:potion",Count:1b,tag:{fireres3:1b,display:{Name:'{"text":"Potion of Fire Resistance","color":"#FFDBDB","italic":false}'},CustomPotionEffects:[{Id:12b,Amplifier:0b,Duration:15600}],Potion:"minecraft:empty",CustomPotionColor:14988413}}}
