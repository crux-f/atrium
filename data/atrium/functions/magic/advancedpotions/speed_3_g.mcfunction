...
execute positioned ~2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{shimmerfeather:1b}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
...
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~2 ~ {NoGravity:1b,Glowing:1b,Motion:[0.0,0.05,0.0],Item:{id:"minecraft:potion",Count:1b,tag:{speed3g:1b,display:{Name:'{"text":"Potion of Swiftness","color":"#FFDBDB","italic":false}'},CustomPotionEffects:[{Id:1b,Amplifier:2b,Duration:900}],Potion:"minecraft:empty",CustomPotionColor:6220774}}}
