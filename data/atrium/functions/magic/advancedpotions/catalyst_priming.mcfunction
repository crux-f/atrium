...
execute positioned ~-2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{id:"minecraft:iron_nugget",Count:1b}}] run scoreboard players add @s cauldron 1
#
# Check NORTH for Mossy Cobblestone
execute positioned ~ ~2 ~-2 at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{id:"minecraft:mossy_cobblestone",Count:1b}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~ ~2 ~-2 if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{id:"minecraft:mossy_cobblestone",Count:1b}}] run scoreboard players add @s cauldron 1
#
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~3 ~ {NoGravity:1b,Glowing:1b,Item:{id:"minecraft:redstone_block",Count:1b,tag:{display:{Name:'{"text":"Potion Catalyst","color":"aqua","italic":false}',Lore:['{"text":"Priming","color":"red","italic":false}','{"text":"Redstone is a natural"}','{"text":"conductor of energy. When"}','{"text":"dusted over growing moss,"}','{"text":"it seems to retain that"}','{"text":"energy for a far greater"}','{"text":"period than previously"}','{"text":"observed."}']},HideFlags:1,catalyst_prime:1b,Enchantments:[{id:"minecraft:power",lvl:1s}]}}}
