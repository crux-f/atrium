...
execute positioned ~2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{charged_blaze_rod:1b}}}] run scoreboard players add @s cauldron 1
#
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~2 ~ {NoGravity:1b,Glowing:1b,Motion:[0.0,0.05,0.0],Item:{id:"minecraft:potion",Count:1b,tag:{strength3g:1b,display:{Name:'{"text":"Potion of Strength","color":"#FFDBDB","italic":false}'},CustomPotionEffects:[{Id:5b,Amplifier:2b,Duration:900}],Potion:"minecraft:empty",CustomPotionColor:6166294}}}
