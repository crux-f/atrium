...
execute positioned ~-2 ~2 ~ at @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{id:"minecraft:splash_potion",Count:1b,tag:{Potion:"minecraft:mundane"}}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~-2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{id:"minecraft:splash_potion",Count:1b,tag:{Potion:"minecraft:mundane"}}}] run scoreboard players add @s cauldron 1
#
...
execute positioned ~2 ~2 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{toxic_branch:1b}}}] run scoreboard players add @s cauldron 1
#
execute if entity @s[scores={cauldron=3..}] at @s run summon item ~ ~2 ~ {NoGravity:1b,Glowing:1b,Motion:[0.0,0.05,0.0],Item:{id:"minecraft:splash_potion",Count:1b,tag:{poison3r:1b,display:{Name:'{"text":"Splash Potion of Poison","color":"#FFDBDB","italic":false}'},CustomPotionEffects:[{Id:19b,Amplifier:1b,Duration:900}],Potion:"minecraft:empty",CustomPotionColor:2866475}}}
