...
execute at @e[tag=fx_travel] positioned ~-6 ~2.5 ~ run particle minecraft:ambient_entity_effect ~ ~ ~ 0.15 0.5 0.15 0.1 10
...
#
# Teleport!
execute in minecraft:overworld at @e[tag=fx_travel] positioned ~-6 ~2 ~ as @e[distance=..1,type=!item_frame,type=!marker,tag=!warping] at @s run function atrium:travel/warp_gate/summon_warp_gate_hook
#
...
execute at @e[tag=fx_travel,scores={warp_gate=..1200}] positioned ~-5 ~5 ~ if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{LodestoneTracked:1b}}}] run schedule function atrium:travel/warp_gate/overworld_tp 1t
