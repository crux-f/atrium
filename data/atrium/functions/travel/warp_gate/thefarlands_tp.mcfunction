...
#
execute at @e[tag=fx_travel] positioned ~ ~2 ~-2 unless entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{tpscroll:1b}}}] run execute at @e[tag=fx_travel] positioned ~-6 ~2 ~ as @e[distance=..1,type=!item_frame] in minecraft:overworld run tp @s -846.5 81 -12547141.5
# Particles!
...
execute if entity @e[tag=fx_travel,scores={warp_gate=..1200}] run schedule function atrium:travel/warp_gate/thefarlands_tp 1t
