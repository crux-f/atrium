# IF there isnt a tpscroll in the CENTER NORTH slot, teleport any player who is in the Gate to 0, 75, 0.
#
execute at @e[tag=fx_travel] positioned ~ ~2 ~-2 unless entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{tpscroll:1b}}}] run execute at @e[tag=fx_travel] positioned ~-6 ~2 ~ as @e[distance=..1,type=!item_frame] in atrium:meridian run tp @s 37013.5 34 -2882.5
# Particles!
...
execute at @e[tag=fx_travel] positioned ~ ~2 ~-2 if entity @e[type=minecraft:item_frame,distance=..1.5,nbt={Item:{tag:{conflux_tws:1b}}}] run execute at @e[tag=fx_travel] positioned ~-6 ~2 ~ as @e[distance=..1,type=!item_frame] in atrium:meridian run function atrium:travel/waystones/conflux_tws_tp
#
