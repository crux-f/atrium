...
execute positioned ~ ~ ~4 if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:stone"}}] run scoreboard players add @s valid_ingredients 1
#
execute if entity @s[scores={valid_ingredients=11..}] at @s run summon item ~ ~1 ~ {NoGravity:1b,Glowing:1b,Item:{id:"minecraft:clay_ball",Count:1b,tag:{display:{Name:'{"text":"Mountain\'s Root","italic":false}',Lore:['{"text":"A culmination of all"}','{"text":"the minerals beneath "}','{"text":"the earth."}']},CustomModelData:333174,atrium_mountains_root:1b}}}
