...
execute positioned ~-2 ~ ~ at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:redstone",Count:1b}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~-2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:redstone",Count:1b}}] run scoreboard players add @s valid_ingredients 1
...
execute if entity @s[scores={valid_ingredients=3..}] at @s positioned ~ ~ ~-7 run advancement grant @p[gamemode=!spectator] only atrium:magic/spacial_awareness
