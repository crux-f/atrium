...
execute positioned ~-2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg",Count:1b}}] run scoreboard players add @s valid_ingredients 1
# Check EAST for Egg
execute positioned ~2 ~ ~ at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg",Count:1b}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:egg",Count:1b}}] run scoreboard players add @s valid_ingredients 1
...
#
execute if entity @s[scores={valid_ingredients=4..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:potion",Count:1b,tag:{display:{Name:'{"text":"Mayo Bottle","italic":false}'},HideFlags:32,CustomModelData:333166,atrium_mayo_bottle:1b,custom_potion_effects:[{id:"minecraft:nausea",amplifier:0b,duration:2400,show_particles:1b,show_icon:1b}],Potion:"minecraft:empty",CustomPotionColor:16383912}}}
