...
execute positioned ~-2 ~ ~ at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:smooth_stone",Count:1b}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~-2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:smooth_stone",Count:1b}}] run scoreboard players add @s valid_ingredients 1
...
#
execute if entity @s[scores={valid_ingredients=3..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:glass_bottle",Count:1b,tag:{display:{Name:'{"text":"Essence of Anachronism","italic":false}',Lore:['{"text":"This substance seems to"}','{"text":"be in the wrong time."}']},CustomModelData:333087,atrium_essence_of_anachronism:1b}}}
