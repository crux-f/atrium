...
execute positioned ~-2 ~ ~ if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:gold_nugget",Count:1b}}] run scoreboard players add @s valid_ingredients 1
...
#
execute if entity @s[scores={valid_ingredients=3..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:glowstone",Count:1b,tag:{display:{Name:'{"text":"Alchemy Catalyst","italic":false}',Lore:['{"text":"Galvanizing","color":"yellow","italic":false}','{"text":" "}','{"text":"When energized by magma,"}','{"text":"glowstone has the ability"}','{"text":"to improve potions beyond"}','{"text":"their typical limitations."}']},atrium_catalyst_galvanizing:1b,Enchantments:[{}]}}}
