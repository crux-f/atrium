...
execute positioned ~ ~ ~-2 at @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:cobweb",Count:1b}}] run particle minecraft:enchanted_hit ~ ~ ~ 0 0 0 0.1 20 normal
execute positioned ~ ~ ~-2 if entity @e[type=minecraft:glow_item_frame,distance=..1.5,nbt={Item:{id:"minecraft:cobweb",Count:1b}}] run scoreboard players add @s valid_ingredients 1
#
execute if entity @s[scores={valid_ingredients=3..}] at @s run summon item ~ ~1 ~ {Motion:[0.0,0.02,0.0],NoGravity:1b,Glowing:1b,Item:{id:"minecraft:potion",Count:1b,tag:{display:{Name:'{"text":"Potion of Slow Falling","italic":false}'},CustomModelData:333067,atrium_longer_slow_falling:1b,atrium_tier_3_potion:1b,custom_potion_effects:[{id:"minecraft:slow_falling",amplifier:0b,duration:9600,show_particles:1b}],Potion:"minecraft:empty",CustomPotionColor:16773073}}}
