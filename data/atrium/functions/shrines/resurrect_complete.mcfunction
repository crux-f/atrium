...
execute unless entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run advancement grant @a[tag=resurrector] only atrium:belief/resurrect_1
execute unless entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run advancement grant @a[tag=resurrector,scores={resurrect_others=10..}] only atrium:belief/resurrect_10
execute unless entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run advancement grant @a[tag=resurrector,scores={resurrect_others=20..}] only atrium:belief/resurrect_20
execute unless entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run advancement grant @a[tag=resurrector,scores={resurrect_others=30..}] only atrium:belief/resurrect_30
...
execute at @p[tag=resurrector] if entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run playsound minecraft:block.conduit.deactivate master @a[distance=..30] ~ ~ ~ 0.5 0.2 0.1
execute at @p[tag=resurrector] if entity @p[tag=resurrectee,distance=..15,gamemode=spectator] run tellraw @p[tag=resurrector] {"text":"But you were not strong enough.","italic":true,"color":"red"}
