...
execute at @s if entity @a[tag=resurrectee,distance=..10] run function atrium:shrines/resurrect_repeat
# If there isn't, remove the resurrector tag from the player and deliver failure message
execute unless entity @a[tag=resurrectee,distance=..15] run tellraw @s {"text":"But there was nobody there.","italic":true,"color":"red"}
execute unless entity @a[tag=resurrectee,distance=..15] run tag @s remove resurrector
