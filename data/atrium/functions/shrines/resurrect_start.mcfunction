...
tag @p[tag=resurrector,nbt={SelectedItem:{id:'minecraft:totem_of_undying'}}] add free_res
# disappear 1 res item from resurrector's inventory
clear @p[tag=resurrector,nbt={SelectedItem:{id:'minecraft:diamond'}}] minecraft:diamond 1
clear @p[tag=resurrector,nbt={SelectedItem:{id:'minecraft:totem_of_undying'}}] minecraft:totem_of_undying 1
...
execute at @p[tag=resurrector] if entity @a[tag=resurrectee,distance=..10] run function atrium:shrines/resurrect_repeat
