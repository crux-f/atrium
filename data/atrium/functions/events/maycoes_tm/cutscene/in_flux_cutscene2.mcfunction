tellraw @a[tag=event] ["",{"text":"[Maycoes]: ","color":"dark_purple"},{"text":"As simple as it is effective.","color":"white"}]
execute at @e[tag=event] run playsound minecraft:atrium.maycoes_talk_1 master @a[tag=event] ~ ~ ~ 0.5 1 1
#
schedule function atrium:events/maycoes_tm/cutscene/in_flux_cutscene3 5s
#
