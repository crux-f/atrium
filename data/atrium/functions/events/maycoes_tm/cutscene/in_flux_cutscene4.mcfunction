tellraw @a[tag=event] ["",{"text":"[Pyronicus]: ","color":"red"},{"text":"Well, well, well! Look who it is!","color":"white"}]
execute at @e[tag=event] run playsound minecraft:entity.skeleton.ambient master @a[tag=event] ~ ~ ~ 5 0.7 1
#
schedule function atrium:events/maycoes_tm/cutscene/in_flux_cutscene5 5s
#
