tellraw @a[tag=event] ["",{"text":"[Pyronicus]: ","color":"red"},{"text":"Is that really all that it takes?","color":"white"}]
execute at @e[tag=event] run playsound minecraft:entity.skeleton.ambient master @a[tag=event] ~ ~ ~ 5 0.7 1
#
schedule function atrium:events/maycoes_tm/cutscene/in_flux_cutscene2 6s
#
...
# execute at @e[tag=event] run playsound minecraft:atrium.maycoes_talk_1 master @a[tag=event] ~ ~ ~ 0.7 1 1
#
