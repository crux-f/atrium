tellraw @a[tag=event] ["",{"text":"[Pyronicus]: ","color":"red"},{"text":"You're going to regret making me crash that airship!","color":"white"}]
execute at @e[tag=event] run playsound minecraft:entity.skeleton.ambient master @a[tag=event] ~ ~ ~ 5 0.7 1
#
schedule function atrium:events/maycoes_tm/cutscene/in_flux_cutscene9 5s
#
