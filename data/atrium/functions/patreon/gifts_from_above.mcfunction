...
execute at @e[tag=sky_gift] run function atrium:summon/fx/summon_poof
execute at @e[tag=sky_gift] positioned ~ ~-400 ~ at @p[distance=..5] run particle dust 50 50 50 2 ~ ~1 ~ 1 1 1 0 50 normal
execute at @e[tag=sky_gift] positioned ~ ~-400 ~ at @p[distance=..5] run playsound minecraft:ui.toast.challenge_complete master @a[distance=..25] ~ ~ ~ 0.5 2 0.01
#
#
# Schedule again
